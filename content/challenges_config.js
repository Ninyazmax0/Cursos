...
const challengesConfig = {
    // NIVEL 3: Semántica HTML
    "level3": {
        title: "Desafío Nivel 3: Semántica HTML",
        subtitle: "Estructura una página usando etiquetas semánticas correctas.",
        instructions: `
            <h2><i data-lucide="layout" class="w-5 h-5 inline mr-2"></i>Instrucciones</h2>
            <p>Mejora la estructura de este documento usando las etiquetas semánticas adecuadas.</p>
            <div class="requirements">
                <div class="requirement-item"><i data-lucide="check-circle" class="w-5 h-5"></i><span>Usa header para la cabecera</span></div>
                <div class="requirement-item"><i data-lucide="check-circle" class="w-5 h-5"></i><span>Usa main para el contenido principal</span></div>
                <div class="requirement-item"><i data-lucide="check-circle" class="w-5 h-5"></i><span>Usa footer para el pie de página</span></div>
                <div class="requirement-item"><i data-lucide="check-circle" class="w-5 h-5"></i><span>Reemplaza los divs genéricos donde corresponda</span></div>
            </div>
        `,
...
<body>
    ...
    <div id="contenido">
        <article>
            ...
            <p>Hola mundo, este es mi primer artículo.</p>
        </article>
    </div>
    ...
    <div id="pie">
        ...
        prompt: "Verifica que el usuario haya reemplazado los <div id='cabecera'> por <header>, <div id='contenido'> por <main>, y <div id='pie'> por <footer>. El uso de <nav> y <article> debe mantenerse.",
        language: "html",
        validate: (code) => {
            const checks = [
                { pattern: /<header/, msg: "Falta la etiqueta <header>" },
                { pattern: /<main/, msg: "Falta la etiqueta <main>" },
                { pattern: /<footer/, msg: "Falta la etiqueta <footer>" }
            ];
            const errors = checks.filter(c => !c.pattern.test(code)).map(c => c.msg);
            const score = Math.max(0, 10 - (errors.length * 3));
            return {
                score: score,
                isPass: score >= 7,
                feedback: errors.length === 0 ? "¡Excelente estructura semántica!" : "Te falta: " + errors.join(', ')
            };
        }
    },
